...
| A-29 | **`_ACCOUNT` records the Snowflake account an extract was *taken from*, not the account each row describes.** For an ACCOUNT_USAGE view the two are the same thing. For an ORGANIZATION_USAGE view they are not: the extract is produced once, in the ORGADMIN-enabled or organization account, and each row names its own account in the view's own `ACCOUNT_NAME` column, which ingest never overwrites. So an organization extract lands with `_ACCOUNT` set to whatever the uploader called that account — the fixture and the demo use the organization name. | ACCOUNT_USAGE views carry no account column at all, so provenance has to be platform metadata (like `_LOADED_AT` and `_BATCH_ID`) rather than source data; adding a synthetic `ACCOUNT_NAME` to the fixture would make it unfaithful and break the registry's column validation (§25, §27.14). Inferring the account from a file name or a directory would be a fabricated dimension (R12). A batch whose account was never declared lands with `_ACCOUNT` NULL — "not recorded", never a guess (R3). | LIVE mode, where the connection itself knows the account and can stamp it without being told; or a Snowflake release that adds an account column to ACCOUNT_USAGE. |
| A-30 | **The fixture's organization money model.** In the generated `USAGE_IN_CURRENCY_DAILY`, every usage type except `data transfer` is denominated in **credits** and priced at the account's currency-per-credit `EFFECTIVE_RATE`; storage is therefore expressed in credits, through `STORAGE_DAILY_HISTORY.CREDITS`. `data transfer` is denominated in **terabytes** and priced per terabyte. Storage is generated at 0.25 credits per TB-day, and effective rates are derived as `edition base rate x regional uplift`. `ON_DEMAND_CONSUMPTION_BALANCE` is generated as the accrued on-demand consumption (a positive number), so `FREE_USAGE_BALANCE + ROLLOVER_BALANCE + CAPACITY_BALANCE - ON_DEMAND_CONSUMPTION_BALANCE` equals the commitment minus cumulative spend. | The verified column lists in the source registry are authoritative and are emitted exactly (§25), but the per-view *unit* and *sign* conventions of the ORGANIZATION_USAGE billing views could not be read first-hand (docs egress blocked, see §6a). What matters for the platform is that the arithmetic is internally exact and asserted — `USAGE x EFFECTIVE_RATE = USAGE_IN_CURRENCY` to the cent, and the burn-down ties to spend to the cent — so a detector or KPI built against the fixture is built against a consistent model. Prices and rates are plausible list figures, not any customer's contract. | First LIVE organization account, or direct access to the ORGANIZATION_USAGE billing reference. The generator's rate and unit constants are module-level in `snowobs_fixtures.organization`, so correcting a convention is a constant change plus the parity tests. |
| A-31 | **Account-scoped sources are deduplicated on their declared grain *plus* `_ACCOUNT`; organization-scoped sources on the declared grain alone.** | An ACCOUNT_USAGE grain is implicitly per-account: `QUERY_ID` and `(WAREHOUSE_ID, START_TIME)` are unique within one account, and two accounts in the same fleet routinely produce the same warehouse ids at the same hours. Without the account in the dedup key, last-write-wins silently discards whole accounts' histories and the roll-up under-reports with no error anywhere. An ORGANIZATION_USAGE grain already names the account, so adding the stamp there would instead break a legitimate re-upload from a different ORGADMIN account. | A source whose declared grain becomes genuinely account-independent, or LIVE mode replacing the offline lake for that view. |
| A-32 | **`SESSION_ID` in the generated `QUERY_HISTORY` is now derived from a SHA-256 digest of the query id rather than Python's `hash()`.** | Python randomises string hashing per process, so the previous value made the generator non-deterministic *between runs* despite a fixed seed — which defeats a fixture whose output is asserted against and made a byte-identity regression guard impossible. The change alters only that opaque column; every other generated file is byte-for-byte unchanged, and the pinned digests in `fixtures/generator/tests/test_organization.py` now hold it in place. | none — this closes a defect rather than opening one. |
| A-33 | **Entities that join two landed tables match on the account as well as on the business key, comparing `COALESCE(ACCOUNT_OF(x), '')` rather than the raw stamp.** The empty string is therefore reserved: an extract landed under an account literally named `''` would be treated as an unrecorded one. | Warehouse names, user names, and dates all repeat across a fleet, so joining `fact_warehouse_metering_hourly`, `fact_storage_daily`, `fact_grant`, and `fact_ai_usage_daily` on the business key alone multiplies one account's rows by the number of accounts sharing that key — a silent fan-out that inflates credits and bytes rather than raising an error. The comparison has to be null-safe because a single-account lake stamps nothing (A-29) and LIVE renders the account as NULL when no connected account is supplied, and `NULL = NULL` would drop every joined row instead. A Snowflake account name is an identifier of at least one character (`dialect_shims._vet_account`), so the empty string cannot collide with a real one. | Ingest begins accepting an account name from an untrusted source without vetting it, or a dialect gains a portable null-safe equality the shim vocabulary can express. |
| A-34 | **`make demo` seeds the four-account organization**, so all 108 KPIs populate and the organization/account filter has something to filter between. `--single-account` seeds one account instead, which leaves the sixteen organization KPIs correctly unavailable — each naming the `ORGANIZATION_USAGE` view it needs — and is kept for testing that path. | Resolved. The single-account generator cannot honestly produce a fleet-wide export: `ORGANIZATION_USAGE` is published from the organization account and names every account, so inventing it from one account's data would be a fabricated dimension (R12). The organization generator produces it properly, and the demo now uploads each account separately — as an enterprise would, since an `ACCOUNT_USAGE` extract carries no account column and the account is recorded at upload time. | The demo's runtime stops fitting the ten-minute budget in §26, at which point the default day count comes down before the account count does: a fleet of one is not this product. |
| A-35 | **An `ACCOUNT_USAGE` metric has no organization-wide figure in LIVE mode.** The scope selector declines it with the reason rather than issuing one query per configured account and merging the results. `ORGANIZATION_USAGE` metrics answer organization-wide in both modes, and OFFLINE rolls up every account in the lake, so this is a LIVE-only limit on operational detail — queries, warehouses, grants, tasks. | An `ACCOUNT_USAGE` view returns one account per connection, so an organization figure is a fan-out plus a merge. The merge is the problem, not the fan-out: summing credits across accounts is sound, but averaging a rate, a percentile, or a share is not, and the semantic layer does not currently declare which measures re-aggregate safely. Shipping a merge that is right for additive measures and quietly wrong for the rest would put an invented number behind an organization label (R12), which is worse than declining. Declining names the alternative — pick an account, or use an `ORGANIZATION_USAGE` metric — so the user is never left without a route to the answer (R3). | The semantic layer gains a per-measure re-aggregation rule (additive / non-additive / weighted-by), at which point the cross-account merge is generated from it rather than assumed. `packages/semantics/src/snowobs_semantics/scope.py::assess` is the single place the refusal is decided, so the change is local. |
| A-36 | **Readiness does not require Postgres or Redis by default.** `READINESS__REQUIRE_POSTGRES` / `READINESS__REQUIRE_REDIS` declare whether a deployment needs each store to serve traffic; both default to `false`, and a component that is not required is reported as `not_required` with its reason rather than probed. The dev compose stack and the AWS deployment set both to `true`. | The API's only consumer of either store is the readiness check itself: the query cache is in-process (`snowobs_engines.cache.ResultCache`), no application code reads the metadata database (A-16, A-18), and Redis is the arq worker's queue — a separate process the all-in-one image does not run. Requiring them unconditionally reported a fully working deployment as `not_ready`: `make demo-native` starts no containers, so its status page carried two red crosses and `/readyz` returned 503 for services nothing uses. Reporting them green instead would have been the same lie inverted — a tick for a check that never ran — so the honest answer is a third state. | **The first Alembic migration for app metadata.** At that point application code reads Postgres, the default flips to `true`, and the ALB begins taking a task out of rotation when its database is unreachable — which it does not do today. The same applies to Redis if the API ever enqueues work or shares the cache. |
