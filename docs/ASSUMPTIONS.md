...
| A-34 | **`make demo` seeds one account, so the twelve organization-scoped KPIs (D10) render as unavailable rather than populated.** 96 of the 108 KPIs are enabled on the demo fixture; the rest each name the `ORGANIZATION_USAGE` view they need. | The single-account generator cannot honestly produce a fleet-wide export: `ORGANIZATION_USAGE` is published from the organization account and names every account in the fleet, so inventing it from one account's data would be a fabricated dimension (R12). The organization generator (`snowobs_fixtures.organization.generate_organization`) does produce it, and the parity fixture and the D10 value tests both land it — what is deferred is only the demo seed script choosing the organization layout, which changes the demo's runtime and its narrative and is owned by the demo phase. | The demo is re-cut to show multi-account roll-up, or a client demo needs the commitment and rate-sheet tiles populated; `scripts/demo_seed.py` then seeds `generate_organization` and ingests the account directories plus the organization directory, exactly as `packages/engines/tests/conftest.py` does. |
