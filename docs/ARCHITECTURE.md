...
| `analytics` | `snowobs_analytics` | `forecast.py` (Theil–Sen trend + seasonality), `anomaly.py` (MAD z-score + deterministic contribution decomposition), `levers.py` (nine optimisation levers), `savings.py` (claim tracking), `alerting.py` (rule model, tiers, dedup ledger, backtest, Snowflake `ALERT` DDL export) |
...
| `chargeback.py` | `GET /api/v1/chargeback/allocation`, `GET /api/v1/chargeback/reconciliation/{usage_date}` |
...
| Worker ships only the `ping` job | Scheduled refresh, reconciliation, close, forecast, and alert-evaluation jobs are not implemented; those engines exist as libraries and are exercised by the API and the tests |
| No alert rule set ships | `packages/analytics/src/snowobs_analytics/alerting.py` implements the rule model, the four tiers, dedup, pruning, backtest, and `ALERT` DDL export. No rules are declared in the repository, and there is no alerts API or UI |
