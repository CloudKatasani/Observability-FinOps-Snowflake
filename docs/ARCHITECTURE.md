...
| No Postgres schema or Alembic migrations | App metadata has no durable store yet. Data-product approvals live in an in-process append-only ledger (A-18) and agent traces in process memory. Because nothing reads or writes Postgres, readiness does not require it by default (`READINESS__REQUIRE_POSTGRES`); the AWS and dev-compose deployments, which provision it deliberately, set it anyway. When the first migration lands this becomes required everywhere |
