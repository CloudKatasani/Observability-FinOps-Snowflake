...
export const componentStatusSchema = z.object({
  name: z.string(),
  /**
   * `not_required` is a third state, not a softer failure: the component is not
   * used by this deployment, so it was never probed. Rendering it as a cross
   * reported a problem the reader could not act on; a tick would claim a check
   * that never ran.
   */
  status: z.enum(["ok", "unavailable", "not_required"]),
...
export const allocationSchema = provenanceSchema.merge(scopeContextSchema).extend({
