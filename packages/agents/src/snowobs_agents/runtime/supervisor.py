...
_ROUTING_KEYWORDS: dict[str, tuple[str, ...]] = {
    ...
    "health": (
        # Deliberately not "fail" or "error": those belong to the SRE agent,
        # which investigates a specific object. These ask whether things are
        # working at all, which is a different question with a different answer
        # shape — state and blast radius rather than root cause.
        "health",
        "healthy",
        "status",
        "broken",
    ...
    "org": (
        # The bare word is the organization signal in a multi-account product:
        # "which account costs the most" and "compare accounts by consumption"
        # otherwise tied with finops and fell to it alphabetically. Questions
        # about ACCOUNTADMIN still reach governance, which matches both the
        # role name and "admin" and so scores higher.
        "account",
        "organization",
        "organisation",
        "org-wide",
